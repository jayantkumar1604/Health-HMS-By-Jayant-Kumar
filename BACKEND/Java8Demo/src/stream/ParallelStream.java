...
public class ParallelStream {
    ...
    public static void main(String[] args) {
        ...
        List<Long> facorialList = list.stream()
                .map(ParallelStream::factorial)
                .toList();

        long endTime = System.currentTimeMillis();
        System.out.println("Time taken with sequentialStream: " + (endTime - startTime) + " ms");

        startTime = System.currentTimeMillis();

        facorialList = list.parallelStream()
                .map(ParallelStream::factorial)
                .toList();

//      facorialList = list.parallelStream().map(ParallelStream::factorial).sequential().toList();
        ...
        System.out.println("Actual result with sequential stream: " + cumulativeSum);
    }
